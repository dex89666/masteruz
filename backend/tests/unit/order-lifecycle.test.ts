...
describe('Критический путь: отмена → возврат эскроу', () => {
  ...
  it('клиент отменяет ACCEPTED (мастер ещё не выехал) — полный возврат, без штрафа', async () => {
    seedUser('client-1', 440_000); // 60000 в эскроу
    store.orders.set('order-a', {
      id: 'order-a',
      clientId: 'client-1',
      masterId: 'master-1',
    ...
    const res = await service.cancelOrder('order-a', 'client-1', 'передумал');

    // Мастер ещё не выехал, контактов не видел → штрафа нет
    expect(res.penaltyAmount).toBe(0);
    expect(store.users.get('client-1')!.balance).toBe(500_000);
    expect(store.orders.get('order-a')!.status).toBe(OrderStatus.CANCELLED);
    expect(store.balanceTx.find((t) => t.type === 'PENALTY')).toBeUndefined();
  });
