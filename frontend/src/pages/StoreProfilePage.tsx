      ...
      <div className="max-w-4xl mx-auto px-4 -mt-12 relative z-10">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex items-start gap-4">
            <div className="w-20 h-20 bg-orange-100 dark:bg-gray-700 rounded-2xl flex items-center justify-center text-3xl shrink-0">
              {store.logoUrl ? <img src={store.logoUrl} alt="" className="w-full h-full rounded-2xl object-cover" /> : <Store size={32} className="text-orange-400" />}
            </div>
              ...
              {store.products.map((product: any) => (
                <div key={product.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
                  {product.imageUrl ? (
                    ...
                    <div className="w-full h-32 bg-gray-100 dark:bg-gray-700 rounded-lg mb-3 flex items-center justify-center"><Package size={24} className="text-gray-400" /></div>
                  )}
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><Star size={20} className="text-amber-500" /> {t('stores.reviewsTitle')}</h2>
            {isAuthenticated && (
              <button
          ...
          {showReviewForm && (
            <div className="bg-white dark:bg-gray-800 rounded-xl p-4 mb-4 shadow-sm">
              <div className="flex items-center gap-2 mb-3">
                {[1, 2, 3, 4, 5].map(star => (
                    <Star key={star} size={24} className={star <= reviewRating ? 'text-amber-500 fill-amber-500 cursor-pointer' : 'text-gray-300 cursor-pointer'} onClick={() => setReviewRating(star)} />
                ))}
              </div>
              <textarea
                value={reviewComment}
              ...
              {store.reviews.map((review: any) => (
                <div key={review.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="flex text-amber-500">{Array.from({ length: review.rating }).map((_, i) => <Star key={i} size={14} className="fill-amber-500" />)}</span>
